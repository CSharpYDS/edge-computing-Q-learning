...
def deepQLearning_v2(job_sequence, time1 = None):
    agent = Agent_v2()
    cost_min, cost_min1 = 10000000, 10000000
    wrong = 0
    ret_history = []
    loss_arr = []
    loss_idx = []
    ...
    plt.savefig('loss_result/temp_server1/' + time1 + str(".png"))
    plt.close()
    torch.save(agent.brain.main_q_network, PATH)
    print("wrong DQN1", wrong)
    return cost_min, cost_min1, ret_history, loss_arr

def eval_v2(job_sequence, time1 = None):
    agent = Agent_v2()
    cost_min, cost_min1 = 10000000, 10000000
    wrong = 0
    ret_history = []
        ...
        for t in range(0, 100000):
            ...
            while True:
                if idx < len(job_sequence) and job_sequence[idx].depart_time == t:
                    ...
                    tensor_state = parseState(state_trans, state_queue, one_hot)
                    # 2.1 获取action
                    action = agent.get_action(tensor_state, 0)
                    server_id = action.cpu().item() # get a number
                    action = action.cuda()
                    # 2.2 执行action
                    job.arrive_time = job.arriveTime(server_id)
